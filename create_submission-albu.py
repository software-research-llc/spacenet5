...
def graphs_to_wkt(graphs, filenames, output_csv_path='',
                   weight_keys=['weight'],#length', 'travel_time_s'],
                   verbose=False):
    """
    ...
    pkl_list = [ os.path.basename(filename) for filename in filenames ]
    for i, G in enumerate(graphs):
        ...
        for i, (u, v, attr_dict) in enumerate(G.edges(data=True)):
            ...
            if (u, v) in seen_edges or (v, u) in seen_edges:
                ...
                seen_edges.add((u, v))
                seen_edges.add((v, u))
            tr_x = lambda x: float(x) * flow.CHIP_CANVAS_SIZE[0] / flow.TARGET_IMSHAPE[0]
            tr_y = lambda x: float(x) * flow.CHIP_CANVAS_SIZE[1] / flow.TARGET_IMSHAPE[1]
            geom_pix_wkt = 'LINESTRING ({} {}'.format(tr_x(attr_dict['pts'][0][0]), tr_y(attr_dict['pts'][0][1]))
            for pt in attr_dict['pts'][1:]:
                geom_pix_wkt += ", {} {}".format( tr_x(pt[0]), tr_y(pt[1]) )
            geom_pix_wkt += ')'
#            geom_pix_wkt = attr_dict['geometry_pix'].wkt
            
            # check edge lnegth
            if attr_dict.get('length', 0) > 5000:
            ...
            if len(weight_keys) > 0:
                weights = [0] + [attr_dict[w] for w in weight_keys]
                if verbose:
    ...
    # create dataframe
    if len(weight_keys) > 0:
        cols = ['ImageId', 'WKT_Pix'] + ['length_m', 'travel_time_s']
    else:
